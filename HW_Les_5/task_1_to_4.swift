...
// 1.  Необходимо создать структуру картошки фри, в которой будет стоимость и размер картошки и сделать так, 
...
class Pizzeria{
    ...
    // Добавить в класс кафе функцию, которая возвращает все позиции меню /// 2 часть
    func get() -> [MenuProtocol]{
        return menu
    }

    func getAvailable(cost: Double) -> [MenuProtocol]{ /// 2 часть
        menuItems.filter{$0.price <= cost}
    }
}